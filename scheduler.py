...
# ---------------------- Периодическое обновление поинтов участников (каждые 5 секунд) ----------------------
async def schedule_points_update(bot: Bot):
    """Запускает периодическое обновление поинтов участников активного раунда каждые 5 секунд"""
    job_id = "points_update"
    if job_id in _scheduled_jobs:
        return
    trigger = IntervalTrigger(seconds=5)  # Интервал 5 секунд
    job = scheduler.add_job(
...
async def start_scheduler(bot: Bot):
    ...
    await schedule_points_update(bot)   # теперь обновление каждые 5 секунд
