...
async def update_user_balance(user_id: int, delta: int):
    """Начисляет (или списывает) поинты пользователю.
    Использует UPSERT — создаёт строку если её нет, иначе обновляет."""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
...
async def add_coins(user_id: int, amount: int):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO user_coins (user_id, total_coins, last_updated)
        VALUES (?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(user_id) DO UPDATE SET
...
async def get_all_users() -> List[Dict]:
    ...
    c = conn.cursor()
    rows = c.execute("""
        SELECT u.user_id, u.username, u.is_banned,
               COALESCE(up.total_points, 0) AS total_points,
               u.exchange_channel_sub, u.exchange_group_sub
        FROM users u
