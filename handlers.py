...
async def callback_current_round(callback: CallbackQuery):
    ...
    if not current_round or current_round['status'] != 'active':
        ...
        await callback.answer()
        return
    # Используем get_channel_by_id, так как current_round['channel_id'] это числовой ID канала
    channel = await db.get_channel_by_id(current_round['channel_id'])
    if not channel:
