...
async def finish_round_and_select_winner(bot: Bot, round_data: dict):
    # Обновляем количество приглашённых
    participants = await update_participant_invite_counts(round_data, bot)
    # Удаляем старое сообщение о старте пиара
    ...
    if not eligible:
        final_text = "🏆 Раунд завершён. Никто не набрал минимальное количество поинтов.\nСледующий раунд будет объявлен позже."
        await bot.send_message(EXCHANGE_CHANNEL_ID, final_text, reply_markup=kb.back_to_main_menu())
    ...
    for i, p in enumerate(eligible, 1):
        ...
        lines.append(f"{i}. @{name} — пригласил {p['points']} чел. → {p['points']} поинтов")
    lines.append(f"\n🏆 *Победитель:* @{await get_username(winner_id)} с {winner_points} поинтами.")
    lines.append("\n🎉 Его канал будет пиариться в следующем раунде (после подтверждения администратором).")
    final_text = "\n".join(lines)
    ...
    # Отправляем победителю запрос на подтверждение
...
async def create_invite_for_participant(msg: Message, user_id: int, channel: dict, bot: Bot, round_data: dict):
    # Название ссылки – юзернейм исполнителя (или его ID)
...
async def admin_get_channel(message: Message, state: FSMContext, bot: Bot):
    ...
    # Сохраняем раунд без start_message_id, он будет добавлен после подтверждения
    await db.create_round(channel_id, start_time, end_time, waiting_for_admin=True, start_message_id=0)
    await state.update_data(channel_id=channel_id, end_time=end_time, title=title, username=username)
    await message.answer(
...
async def admin_confirm_round(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if not channel:
        ...
        await callback.answer()
        return
    # Отправляем сообщение в канал (а не в группу)
    # Используем EXCHANGE_CHANNEL_ID – это ID вашего канала @aterusmm
    text = (
        f"🎉 *Старт пиара!*\n\n"
    ...
    # Клавиатура: одна кнопка с callback_data, содержащим channel_id и round_id
    ...
    await callback.message.edit_text(
        f"✅ Раунд запущен! Сообщение отправлено в канал.\nОкончание: {current['end_time']}"
    )
    await callback.answer()
...
async def join_round_callback(callback: CallbackQuery, bot: Bot):
    # Извлекаем round_id (хотя он всегда 1) и channel_id
    ...
    if not channel:
        ...
        return
    # Проверяем подписку пользователя на канал заказчика
    ...
    if not subscribed:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="✅ Я подписался", callback_data=f"retry_join_{round_id}_{channel_id}")]
        ])
        await callback.message.answer(
            f"❌ Вы не подписаны на канал @{channel['username']}.\n"
            f"Пожалуйста, подпишитесь и нажмите «Я подписался».",
            reply_markup=keyboard
        )
        await callback.answer()
        return
    # Подписан – создаём пригласительную ссылку
    link_name = f"@{callback.from_user.username}" if callback.from_user.username else str(user_id)
    expire_date = datetime.now() + timedelta(hours=INVITE_LINK_EXPIRE_HOURS, minutes=INVITE_LINK_EXTRA_MINUTES)
    try:
        invite_link = await bot.create_chat_invite_link(
            chat_id=channel['channel_id'],
        ...
        await callback.message.answer(f"Ошибка создания ссылки: {e}")
        await callback.answer()
        return
    await db.add_participant(user_id, invite_link.invite_link, link_name)
    ...
    await callback.message.answer("✅ Вы участвуете в пиаре! Проверьте личные сообщения от бота для получения ссылки.")
    await callback.answer()
...
async def retry_join_callback(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        pass
    await callback.message.edit_text("✅ Вы участвуете! Ссылка отправлена в ЛС.")
    await callback.answer()

# ---------------------- Остальные админ-функции (списки, жалобы, история) ----------------------
