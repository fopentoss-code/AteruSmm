...
async def admin_channel_selected(message: Message, state: FSMContext, bot: Bot):
    ...
    # КРИТИЧЕСКАЯ ПРОВЕРКА: канал должен быть закрытым (приватным) — без username
...
async def admin_confirm_round(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # Если канал закрытый, у него нет username – используем ссылку-приглашение
...
# ---------------------- Обработка отклика на пиар (с начислением 1 поинта исполнителю за подписку) ----------------------
...
# ---------------------- Обработчик заявок на вступление (начисление поинтов за приглашённых) ----------------------
...
async def handle_join_request(join_request: ChatJoinRequest, bot: Bot):
    ...
    if invite_link:
        ...
        print(f"🏷 Имя ссылки: {invite_link.name}")
    else:
        print("❌ Нет объекта invite_link")
        return
    ...
    if not invite_link or not invite_link.name:
        print("❌ Нет имени ссылки или ссылка отсутствует, поинт не начислен")
        return

    # Общая ссылка начала раунда — не начисляем
    if invite_link.name == "common_for_round":
        print("ℹ️ Это общая ссылка начала раунда, поинт не начисляется")
        return

    try:
        inviter_id = int(invite_link.name)
    except ValueError:
        print(f"❌ Имя ссылки не число: {invite_link.name}, поинт не начислен")
        return
    ...
    print(f"✅ Начислен 1 поинт пользователю {inviter_id} за приглашение")
    ...
    if current_round and current_round['status'] == 'active':
        ...
        if participant:
            ...
            print(f"✅ Обновлены очки участника {inviter_id} в раунде: {new_points}")
        else:
            print(f"⚠️ Участник {inviter_id} не найден в round_participants")
    else:
        print(f"⚠️ Нет активного раунда или раунд не в статусе active")
