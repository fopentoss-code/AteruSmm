...
async def handle_join_request(join_request: ChatJoinRequest, bot: Bot):
    ...
    if not invite_link or not invite_link.name:
        # Всё равно одобрили, но поинт не начисляем
        return
    ...
    try:
        ...
        # Если название не число – возможно, это username, ищем в БД
        inviter_id = None
        if invite_link.name.startswith("@"):
            # Можно попробовать найти пользователя по username (требует отдельную функцию в database.py)
            pass
        if not inviter_id:
            return
    ...
    # Дополнительно можно обновить member_count ссылки (но это не обязательно, т.к. событие уже обработано)
