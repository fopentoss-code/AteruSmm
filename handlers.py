...
async def finish_round_and_select_winner(bot: Bot, round_data: dict):
    ...
    if not eligible:
        ...
        await bot.send_message(EXCHANGE_CHANNEL_ID, final_text, reply_markup=kb.balance_button())
...
# ---------------------- Обработка отклика на пиар (без публикаций в канале) ----------------------
...
async def join_round_callback(callback: CallbackQuery, bot: Bot):
    ...
    # Отправляем ссылку в личные сообщения
    try:
        await bot.send_message(
            user_id,
