...
class CreateRoundStates(StatesGroup):
    ...
    waiting_for_channel_selection = State()  # новое состояние для выбора канала через кнопку
...
async def send_admin_round_stats(bot: Bot, round_data: dict, participants: list, winner_id: int = 0):
    ...
    channel_name = f"@{channel['username']}" if channel else f"ID {round_data['channel_id']}"
...
# ---------------------- Главное меню и основные кнопки (с редактированием) ----------------------
...
# ----- СОЗДАНИЕ РАУНДА ЧЕРЕЗ КНОПКУ ВЫБОРА КАНАЛА -----
...
async def admin_create_round(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "Система автоматически запросит у вас права на добавление бота администратором.",
        reply_markup=markup
    )
...
async def admin_channel_selected(message: Message, state: FSMContext, bot: Bot):
    ...
    # Получаем информацию о канале через бота
    try:
    ...
    # Проверяем, что бот имеет права администратора (после добавления через интерфейс выбора)
    bot_is_admin = await db.check_bot_admin_rights(channel_id, bot)
    if not bot_is_admin:
        await message.answer(
            "❌ Бот не является администратором выбранного канала или у него нет права приглашать участников.\n"
            "Пожалуйста, при выборе канала предоставьте боту права администратора."
        )
        await state.clear()
        await message.answer("Отменено.", reply_markup=ReplyKeyboardMarkup(keyboard=[], resize_keyboard=True))
        return
    ...
    # Убираем клавиатуру с кнопкой выбора
    await message.answer(
...
async def admin_confirm_round(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    text = (
        f"🎉 *Старт пиара!*\n\n"
        f"🔗 Канал: [{channel['username']}](https://t.me/{channel['username']})\n"
...
# ---------------------- Обработка отклика на пиар (создание ссылки для закрытого канала) ----------------------
...
async def join_round_callback(callback: CallbackQuery, bot: Bot):
    ...
    # СОЗДАНИЕ ССЫЛКИ ДЛЯ ЗАКРЫТОГО КАНАЛА (creates_join_request=True)
    link_name = str(user_id)  # сохраняем ID пригласившего в названии ссылки
    ...
    try:
        invite_link = await bot.create_chat_invite_link(
            ...
            member_limit=1,               # обязательное условие для creates_join_request
            creates_join_request=True     # режим заявок
        )
    except Exception as e:
    ...
    try:
        await bot.send_message(
            user_id,
            f"✅ Вы успешно откликнулись на пиар канала @{channel['username']}!\n\n"
...
async def retry_join_callback(callback: CallbackQuery, bot: Bot):
    ...
    if existing:
        ...
        if invite_link:
            try:
                await bot.send_message(
                    user_id,
                    f"✅ Вы уже откликнулись на пиар канала @{channel['username']}!\n\n"
    ...
    subscribed = await check_user_subscribed_to_channel(user_id, channel['channel_id'], bot)
    if not subscribed:
        await callback.answer(f"❌ Вы всё ещё не подписаны на @{channel['username']}.", show_alert=True)
        return

    # Создаём ссылку для закрытого канала
...
# ---------------------- НОВЫЙ ОБРАБОТЧИК ЗАЯВОК НА ВСТУПЛЕНИЕ (для закрытых каналов) ----------------------
@router.chat_join_request()
async def handle_join_request(join_request: ChatJoinRequest, bot: Bot):
    """
    Обрабатывает заявку на вступление в закрытый канал.
    Автоматически одобряет заявку, начисляет поинт пригласившему и обновляет статистику раунда.
    """
    invite_link = join_request.invite_link
    user_id = join_request.from_user.id
    username = join_request.from_user.username or join_request.from_user.first_name

    # Сначала одобряем заявку, чтобы пользователь попал в канал
    try:
    ...
    # Если ссылка не от бота или не содержит названия – выходим
    if not invite_link or not invite_link.name:
        return

    # Название ссылки – это ID пригласившего (мы сохраняли его как str(user_id))
    try:
        inviter_id = int(invite_link.name)
    except ValueError:
        return

    # Начисляем поинт пригласившему
    ...
    # Обновляем статистику в текущем раунде
    current_round = await db.get_current_round()
