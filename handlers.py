...
async def admin_confirm_round(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # === СОЗДАНИЕ ОБЩЕЙ ПРИГЛАСИТЕЛЬНОЙ ССЫЛКИ ДЛЯ СООБЩЕНИЯ ===
    # Создаём ссылку, которая будет отображаться в сообщении о старте пиара
    try:
        common_link = await bot.create_chat_invite_link(
            chat_id=channel['channel_id'],
            name="common_for_round",  # общее название
            ...
            creates_join_request=True  # для закрытых каналов
        )
    ...
    # Формируем текст сообщения, включая общую ссылку
    if channel.get('username'):
        channel_display = f"https://t.me/{channel['username']}"
    else:
        channel_display = common_link_url  # если нет username, используем созданную ссылку
...
async def join_round_callback(callback: CallbackQuery, bot: Bot):
    ...
    if not subscribed:
        # Всплывающее сообщение (alert) как просил пользователь
        if channel.get('username'):
    ...
    # Создаём уникальную пригласительную ссылку для исполнителя
    link_name = str(user_id)  # сохраняем ID пригласившего
    expire_date = datetime.now() + timedelta(hours=INVITE_LINK_EXPIRE_HOURS, minutes=INVITE_LINK_EXTRA_MINUTES)
    try:
...
async def handle_join_request(join_request: ChatJoinRequest, bot: Bot):
    ...
    # Из названия ссылки получаем ID пригласившего
    try:
    ...
    # Начисляем 1 поинт пригласившему
