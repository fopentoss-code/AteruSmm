...
async def send_admin_round_stats(bot: Bot, round_data: dict, participants: list, winner_id: int = 0):
    ...
    duration = end_time - start_time if isinstance(start_time, datetime) else "—"
    lines = [
    ...
    for i, p in enumerate(participants, 1):
        ...
        round_pts = p.get('points', 0)
        total_pts = p.get('total_points', await db.get_user_points(p['user_id']))
        lines.append(f"{i}. @{name} — {total_pts} поинтов всего (в этом раунде: +{round_pts})")
    if winner_id != 0:
...
async def finish_round_and_select_winner(bot: Bot, round_data: dict):
    """Завершает раунд и выбирает победителя по ОБЩЕМУ балансу поинтов за всё время."""

    # Удаляем стартовое сообщение из канала биржи
    ...
    # Подтягиваем ОБЩИЙ баланс поинтов для каждого участника
    # Победитель определяется по общему балансу за всё время, не только за этот раунд
    for p in participants:
        p['total_points'] = await db.get_user_points(p['user_id'])

    # Сортируем по общему балансу
    participants.sort(key=lambda x: x['total_points'], reverse=True)

    # Отбираем тех, у кого достаточно поинтов
    eligible = [p for p in participants if p['total_points'] >= MIN_POINTS_TO_WIN]

    # Топ-10 для публикации тир-листа
    top_10 = participants[:10]
    lines = ["📊 *Тир-лист раунда (топ-10 по общему балансу поинтов):*\n"]
    for i, p in enumerate(top_10, 1):
        user = await db.get_user(p['user_id'])
        name = user.get('username') if user else str(p['user_id'])
        round_pts = p.get('points', 0)
        total_pts = p['total_points']
        lines.append(f"{i}. @{name} — {total_pts} поинтов (в этом раунде: +{round_pts})")
    ...
    lines.append(f"\n🏆 *Победитель:* @{await get_username(winner_id)} с {winner_total_points} поинтами.")
    ...
    history_id = await db.add_round_history(
        ...
        winner_points=winner_total_points,
        start_time=round_data['start_time'],
        end_time=datetime.now(),
        status='completed',
    ...
    try:
        await bot.send_message(
            winner_id,
            f"🎉 Поздравляем! Вы заняли первое место в конкурсе пиара с {winner_total_points} поинтами.\n\n"
            ...
            f"Если вы откажетесь, ваши поинты будут конвертированы в коины (1 поинт = 1 коин), "
            f"которые можно будет использовать в магазине бота.",
...
async def admin_channel_selected(message: Message, state: FSMContext, bot: Bot):
    ...
    if chat.username is not None:
        await message.answer(
            "❌ Канал должен быть ЗАКРЫТЫМ (приватным).\n\n"
            "Как исправить: Настройки канала → Тип канала → Выбрать «Закрытый канал».\n"
            "После этого повторите выбор канала."
        )
        await state.clear()
        await message.answer("Отменено.", reply_markup=ReplyKeyboardMarkup(keyboard=[], resize_keyboard=True))
        return
    ...
    if bot_member.status == 'administrator' and not bot_member.can_invite_users:
        await message.answer("❌ У бота нет права 'Приглашать пользователей'. Выдайте это право при добавлении бота в администраторы.")
        await state.clear()
        await message.answer("Отменено.", reply_markup=ReplyKeyboardMarkup(keyboard=[], resize_keyboard=True))
        return
...
async def admin_confirm_round(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        sent_msg = await bot.send_message(EXCHANGE_CHANNEL_ID, text, parse_mode="HTML", reply_markup=keyboard)
...
async def join_round_callback(callback: CallbackQuery, bot: Bot):
    ...
    # Начисляем 1 поинт исполнителю за подписку
    await db.update_user_balance(user_id, 1)
    print(f"Начислен 1 поинт исполнителю {user_id} за подписку на канал {channel['channel_id']}")

    # Создаём уникальную пригласительную ссылку (имя ссылки = user_id исполнителя)
    ...
    # Добавляем 1 поинт в статистику текущего раунда (за саму подписку)
    participant = await db.get_participant(user_id)
    if participant:
        await db.update_participant_points(user_id, 1)

    channel_name = f"@{channel['username']}" if channel.get('username') else "канала"
    try:
        await bot.send_message(
            user_id,
            f"✅ Вы успешно откликнулись на пиар {channel_name}!\n\n"
            f"Вы получили +1 поинт за подписку.\n\n"
...
async def retry_join_callback(callback: CallbackQuery, bot: Bot):
    ...
    channel = await db.get_channel_by_id(channel_id)
    channel_name = f"@{channel['username']}" if channel and channel.get('username') else "канала"
    ...
    if existing and existing.get('invite_link'):
        try:
            await bot.send_message(
                user_id,
                f"✅ Вы уже откликнулись на пиар {channel_name}!\n\n"
...
# ---------------------- Обработчик заявок на вступление (ChatJoinRequest) ----------------------
@router.chat_join_request()
async def handle_join_request(join_request: ChatJoinRequest, bot: Bot):
    print(f"🔔 Получен запрос на вступление от {join_request.from_user.id} (@{join_request.from_user.username})")

    invite_link_obj = join_request.invite_link

    if invite_link_obj:
        print(f"🔗 Ссылка: {invite_link_obj.invite_link}")
        print(f"🏷 Имя ссылки: {invite_link_obj.name}")
    else:
        print("❌ Нет объекта invite_link — одобряем без начисления поинтов")
        try:
    ...
    # Регистрируем вступившего пользователя в системе
    await db.register_user(join_request.from_user.id, join_request.from_user.username)
    ...
    # Общая ссылка старта раунда — поинт не начисляем
    if not invite_link_obj.name or invite_link_obj.name == "common_for_round":
        print("ℹ️ Общая ссылка раунда или имя не задано — поинт не начисляется")
        return

    # Имя ссылки = user_id пригласившего исполнителя
    try:
        inviter_id = int(invite_link_obj.name)
    except ValueError:
        print(f"❌ Имя ссылки не является числом: {invite_link_obj.name}")
        return

    inviter = await db.get_user(inviter_id)
    if not inviter:
        print(f"❌ Пригласивший {inviter_id} не найден в БД")
        return

    username = join_request.from_user.username or join_request.from_user.first_name

    # Начисляем +1 поинт пригласившему в общий баланс
    await db.update_user_balance(inviter_id, 1)
    print(f"✅ Начислен 1 поинт пользователю {inviter_id} за приглашение @{username}")
    ...
    try:
        ...
        print(f"⚠️ Не удалось отправить уведомление пригласившему: {e}")
    ...
    if current_round and current_round['status'] == 'active':
        ...
        if participant:
            new_round_points = participant['points'] + 1
            await db.update_participant_points(inviter_id, new_round_points)
            print(f"✅ Обновлены очки участника {inviter_id} в раунде: {new_round_points}")
        else:
            print(f"⚠️ Участник {inviter_id} не найден в round_participants")
    else:
        print(f"⚠️ Нет активного раунда или раунд не активен")
...
async def winner_accept(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    button = KeyboardButton(text="📢 Выбрать свой канал для пиара", request_chat=request_channel)
    markup = ReplyKeyboardMarkup(keyboard=[[button]], resize_keyboard=True, one_time_keyboard=True)

    await callback.message.answer(
        "🎉 Отлично! Для запуска пиара вашего канала выполните следующие условия:\n\n"
        "1️⃣ Канал должен быть <b>ЗАКРЫТЫМ (приватным)</b>\n"
        "   Настройки канала → Тип канала → Закрытый канал\n\n"
        "2️⃣ Добавьте бота администратором своего канала\n"
        "   и выдайте ему <b>все права</b> (особенно «Приглашать пользователей»)\n\n"
        "После выполнения условий нажмите кнопку ниже и выберите свой канал:",
        parse_mode="HTML",
        reply_markup=markup
    )
    await state.update_data(history_id=pending['round_history_id'])
    await state.set_state(WinnerSetupStates.waiting_for_channel_selection)
    await callback.answer()
...
async def winner_decline(callback: CallbackQuery, bot: Bot):
    ...
    await bot.send_message(
        EXCHANGE_CHANNEL_ID,
        f"⚠️ Победитель @{username} отказался от пиара. Его поинты конвертированы в коины."
    )
    ...
    await callback.answer()
    await offer_next_winner(bot, declined_user_id=user_id)

@router.message(WinnerSetupStates.waiting_for_channel_selection, F.chat_shared)
async def winner_channel_selected(message: Message, state: FSMContext, bot: Bot):
    user_id = message.from_user.id
    shared_chat = message.chat_shared
    if not shared_chat:
        await message.answer("❌ Не удалось получить информацию о канале. Нажмите кнопку ещё раз.")
        return
    ...
    try:
        ...
        await message.answer(f"❌ Не удалось получить данные о канале: {e}\nПопробуйте снова.")
        return

    # Проверка: это канал, а не группа
    if chat.type != "channel":
        await message.answer("❌ Выберите канал, а не группу. Нажмите кнопку снова.")
        return

    # Проверка: канал должен быть закрытым (без username)
    if channel_username is not None:
        request_channel = KeyboardButtonRequestChat(
            request_id=2, chat_is_channel=True, bot_is_member=False, bot_administrator_rights=None
        )
        markup = ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text="📢 Выбрать свой канал для пиара", request_chat=request_channel)]],
            resize_keyboard=True, one_time_keyboard=True
        )
        await message.answer(
            "❌ Канал должен быть <b>ЗАКРЫТЫМ (приватным)</b>.\n\n"
            "Настройки канала → Тип канала → Закрытый канал.\n\n"
            "После изменения нажмите кнопку снова:",
            parse_mode="HTML",
            reply_markup=markup
        )
        return

    # Проверка: бот является администратором
    try:
        bot_member = await bot.get_chat_member(channel_id, bot.id)
        if bot_member.status not in ['administrator', 'creator']:
            request_channel = KeyboardButtonRequestChat(
            ...
            await message.answer(
                ...
                "Добавьте бота в администраторы и выдайте ему все права.\n\n"
        ...
        if bot_member.status == 'administrator' and not bot_member.can_invite_users:
            ...
            await message.answer(
                "❌ У бота нет права «Приглашать пользователей».\n\n"
                "Откройте права бота в настройках канала и включите это право.\n\n"
                "После изменения нажмите кнопку снова:",
                reply_markup=markup
            )
            return
    except Exception as e:
        await message.answer(f"❌ Ошибка при проверке прав бота: {e}\nПопробуйте снова.")
        return

    # Все условия выполнены — сохраняем канал и запускаем раунд
    await db.add_channel(user_id, channel_id, channel_username, title, True)
    await db.reset_user_points(user_id)
    await db.clear_pending_winner()

    start_time = datetime.now()
    end_time = start_time + timedelta(minutes=ROUND_DURATION_MINUTES)
    await db.create_round(channel_id, start_time, end_time, waiting_for_admin=False, start_message_id=0)

    # Создаём общую пригласительную ссылку для раунда
    try:
        common_link = await bot.create_chat_invite_link(
            chat_id=channel_id,
            name="common_for_round",
        ...
        await message.answer(
            f"❌ Не удалось создать пригласительную ссылку: {e}\n"
            f"Убедитесь что бот имеет право «Приглашать пользователей», и попробуйте снова.",
    ...
    text = (
        f"🎉 <b>Старт пиара!</b>\n\n"
        f"🔗 Канал: {common_link_url}\n"
        f"📅 Продолжительность: {ROUND_DURATION_MINUTES} минут.\n\n"
        f"👇 Нажмите кнопку ниже, чтобы участвовать и зарабатывать поинты!\n"
        f"Для участия необходимо быть подписанным на канал."
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Откликнуться", callback_data=f"join_round_{current_round['id']}_{channel_id}")]
    ])
    ...
    try:
        ...
        await message.answer(
            f"❌ Не удалось отправить сообщение в канал биржи: {e}",
            reply_markup=ReplyKeyboardMarkup(keyboard=[], resize_keyboard=True)
        )
        await db.delete_current_round()
        await state.clear()
    ...
    await message.answer(
        f"✅ Пиар вашего канала «{title}» запущен!\n"
        f"Раунд продлится {ROUND_DURATION_MINUTES} минут.\n\n"
        f"Следите за результатами в канале.",
        reply_markup=ReplyKeyboardMarkup(keyboard=[], resize_keyboard=True)
    )

    await bot.send_message(
        ADMIN_ID,
        f"🏆 Победитель @{message.from_user.username or user_id} запустил пиар канала «{title}».\n"
        f"Раунд завершится: {end_time}"
    )
...
async def winner_timeout(bot: Bot, user_id: int, history_id: int):
    ...
    if pending and pending['user_id'] == user_id:
        ...
        await bot.send_message(
            EXCHANGE_CHANNEL_ID,
            f"⏰ Победитель (id:{user_id}) не ответил вовремя. Поинты конвертированы в коины."
        )
        await offer_next_winner(bot, declined_user_id=user_id)


# ---------------------- Предложение пиара следующему участнику ----------------------
async def offer_next_winner(bot: Bot, declined_user_id: int):
    """После отказа/таймаута победителя предлагает пиар следующему по тир-листу."""
    all_users = await db.get_all_users()
    eligible = [
        u for u in all_users
        if u['user_id'] != declined_user_id
        and u['total_points'] >= MIN_POINTS_TO_WIN
    ...
    if not eligible:
        await bot.send_message(
            EXCHANGE_CHANNEL_ID,
            "🏆 Больше нет участников с достаточным количеством поинтов для пиара."
        )
        return

    next_winner = eligible[0]
    next_user_id = next_winner['user_id']
    next_points = next_winner['total_points']

    last_history = await db.get_last_round_history()
    history_id = last_history['id'] if last_history else 0

    expires_at = datetime.now() + timedelta(minutes=CONFIRMATION_TIMEOUT_MINUTES)
    await db.set_pending_winner(next_user_id, history_id, expires_at)

    try:
        await bot.send_message(
            next_user_id,
            f"🎉 Поздравляем! Предыдущий победитель отказался от пиара.\n"
            f"Теперь вы на первом месте с <b>{next_points}</b> поинтами!\n\n"
            f"Желаете ли вы обнулить ваш текущий баланс поинтов и начать пиар своего канала?\n"
            f"У вас есть <b>{CONFIRMATION_TIMEOUT_MINUTES} минут</b> на решение.\n\n"
            f"Если вы откажетесь, ваши поинты будут конвертированы в коины (1 поинт = 1 коин).",
            parse_mode="HTML",
            reply_markup=kb.winner_confirmation_keyboard()
        )
    except Exception as e:
        print(f"Не удалось отправить предложение следующему победителю {next_user_id}: {e}")
        await db.clear_pending_winner()
        return

    from scheduler import schedule_winner_timeout
    await schedule_winner_timeout(bot, next_user_id, history_id)

    next_username = next_winner.get('username') or str(next_user_id)
    await bot.send_message(
        EXCHANGE_CHANNEL_ID,
        f"📩 Предложение пиара отправлено следующему участнику @{next_username}."
    )
